execute unless data storage ajjnn:data version run tellraw @s [{text:"Unable perform forward pass; ",color:"red"},{text:"ajjnn",color:"gray"},{text:" is not installed",color:"green",click_event:{action:"suggest_command",command:"/function ajjnn:__install"},hover_event:{action:"show_text",value:"Click Here"}}]
...
execute if data storage ajjnn:data {modules:[]} run tellraw @s {text:"Unable to perform forward pass; model was not found",color:"red"}
