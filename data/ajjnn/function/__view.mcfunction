execute unless data storage ajjnn:data version run tellraw @s [{text:""},{text:"Unable to view the model architecture; ",color:"red"},{text:"ajjnn",color:"gray"},{text:" is not ",color:"red"},{text:"installed",color:"green",click_event:{action:"suggest_command",command:"/function ajjnn:__install"},hover_event:{action:"show_text",value:"Click Here"}}]
execute unless data storage ajjnn:data version run return fail

execute if data storage ajjnn:data {modules:[]} run tellraw @s {text:"Unable to view the model architecture; the model is not loaded",color:"red"}
